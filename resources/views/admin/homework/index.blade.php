<x-app-layout>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">{{ __('Manage and track homework assignments across all classes.') }}</p>
        </div>
        <a href="{{ route('admin.homework.create') }}" class="btn-primary text-sm">
                    ...
                    <tbody>
                        @foreach($homeworks as $hw)
                            <tr class="border-b border-gray-100 hover:bg-gray-50/50">
                                ...
                                <td class="px-4 py-3 text-gray-600">{{ $hw->teacher->user->name ?? 'Admin' }}</td>
        ...
        @else
            <div class="p-8 text-center">
                <div class="flex flex-col items-center">
                    ...
                    <p class="text-gray-500 text-sm max-w-sm mb-4">{{ __('Create homework assignments for students to complete outside of class hours.') }}</p>
                    <a href="{{ route('admin.homework.create') }}" class="btn-primary text-sm py-2 px-4">{{ __('Create Assignment') }}</a>
                </div>
            </div>
        @endif
    </x-ui.card>

<x-app-layout>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="bg-white rounded-xl p-4 border border-gray-100 flex items-center gap-4 shadow-sm">
            ...
            <div>
                ...
                <p class="font-bold text-gray-900">{{ $student->german_level->value ?? 'Pending' }}</p>
            </div>
        </div>
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        @forelse($feed as $item)
            @if($item['type'] === 'live')
                <div class="bg-white rounded-2xl border-2 border-eed-200 p-6 shadow-md shadow-red-100/50 relative overflow-hidden">
                    ...
                    <div class="flex items-start justify-between relative z-10">
                        <div>
                            ...
                            <p class="text-gray-500">Started {{ $item['data']->started_at->diffForHumans() }}</p>
                        </div>
            ...
            @elseif($item['type'] === 'next_lesson')
                <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm flex items-center gap-6">
                    ...
                    <div class="flex-1">
                        <p class="text-xs font-semibold text-blue-600 uppercase tracking-wider mb-1">Upcoming Today</p>
                        ...
                        <div class="flex-1">
                            <div class="flex items-start justify-between">
                                ...
                                <span class="text-xs font-medium px-2.5 py-1 rounded-full {{ $item['data']->deadline->diffInHours() < 24 ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700' }} whitespace-nowrap">
                                    Due {{ $item['data']->deadline->diffForHumans() }}
                                </span>
                            </div>
                            <div class="mt-4 pt-4 border-t border-gray-100 flex justify-end gap-3">
                                <a href="#" class="btn-primary text-sm py-1.5 px-4">Submit Assignment</a>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex gap-4 items-center">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">{{ $item['data']->time_limit_minutes ? $item['data']->time_limit_minutes . ' mins • ' : '' }}{{ $item['data']->questions()->count() }} questions</p>
                            </div>
                        </div>
                        <a href="#" class="btn bg-purple-600 hover:bg-purple-700 text-white text-sm py-1.5 px-4">Start Quiz</a>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm relative overflow-hidden">
                    @if($item['data']->is_pinned)
                        <div class="absolute top-0 end-0 w-16 h-16 overflow-hidden">
                            <div class="bg-accent text-white text-[10px] font-bold uppercase tracking-wider text-center py-1 absolute rotate-45 w-24 end-[-24px] top-[12px] shadow-sm">Pinned</div>
                        </div>
                    @endif
                    <div class="flex gap-4">
        ...
        @empty
            <div class="bg-white rounded-2xl border border-gray-200 p-12 text-center shadow-sm">
                ...
                <p class="text-gray-500 max-w-sm mx-auto">No pending homework, quizzes, or upcoming lessons today. Enjoy your free time!</p>
            </div>
        @endforelse
    </div>
</x-app-layout>

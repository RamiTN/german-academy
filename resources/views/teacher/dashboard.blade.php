<x-app-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <x-ui.stat-card 
            ...
            icon="document-text" 
            trend="{{ $stats['pending_homework'] > 0 ? 'Needs attention' : 'All caught up' }}"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <!-- Today's Lessons -->
        <x-ui.card header="Today's Lessons">
            <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between p-4 rounded-xl border {{ $lesson->isLive() ? 'border-red-200 bg-red-50' : 'border-gray-100 bg-gray-50' }}">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">{{ $lesson->classGroup->name }} • {{ $lesson->classGroup->students()->count() }} students</p>
                                @if($lesson->isLive())
                        ...
                        <div>
                            @if($lesson->isLive())
                                <button class="btn-danger text-sm">End Lesson</button>
                            @elseif($lesson->isEnded())
                                <span class="text-sm font-medium text-gray-500">Completed</span>
                            @else
                                <button class="btn-primary text-sm">Start Lesson</button>
                            @endif
                        </div>
                    </div>
                @empty
        ...
        <!-- Recent Submissions -->
        <x-ui.card header="Recent Submissions to Grade">
            <div class="space-y-4">
                @forelse($recentSubmissions as $submission)
                    <div class="flex items-center justify-between p-4 rounded-xl border border-gray-100 hover:border-accent hover:shadow-sm transition-all">
                        ...
                        <div class="text-end">
                            ...
                            <a href="#" class="text-sm font-medium text-accent hover:text-accent-hover">Grade &rarr;</a>
                        </div>
                    </div>
                @empty
                    <div class="text-center py-8 text-gray-500">
                        <p>All caught up on grading!</p>
                    </div>
                @endforelse
            </div>
        </x-ui.card>

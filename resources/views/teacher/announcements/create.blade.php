<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
            ...
            <a href="{{ route('teacher.announcements.index') }}" class="btn-secondary text-sm">&larr; {{ __('Back') }}</a>
        </div>
            ...
            <form action="{{ route('teacher.announcements.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="class_group_id" id="class_group_id" class="input">
                        <option value="">{{ __('All My Classes (Global)') }}</option>
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" required class="input" placeholder="e.g. Class Rescheduled to Friday">
                    @error('title')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="content" class="label">{{ __('Content') }}</label>
                    <textarea name="content" id="content" rows="5" required class="input" placeholder="Write your announcement message here..."></textarea>

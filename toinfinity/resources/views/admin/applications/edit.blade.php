<x-app-layout>
    <x-slot:header>{{ __('Review Application') }}</x-slot>

    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">{{ __('Review & Approve Application') }}</h2>
            <p class="text-sm text-gray-500 mt-1">{{ __('Review applicant details, assign class group, and update application status.') }}</p>
        </div>
        <a href="{{ route('admin.applications.index') }}" class="btn-secondary text-sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <x-ui.card header="{{ __('Applicant Details') }}">
                <div class="space-y-4">
                    ...
                    <div class="pt-4 border-t border-gray-100">
                        <p class="text-sm text-gray-500 font-medium mb-1">{{ __('Preferred Schedule / Notes') }}</p>
                    ...
                    @if($application->reviewer)
                        <div class="pt-4 border-t border-gray-100 text-xs text-gray-400">
                            {{ __('Reviewed by') }}: {{ $application->reviewer->name }} {{ __('on') }} {{ $application->reviewed_at ? $application->reviewed_at->format('Y-m-d H:i') : '' }}
                        </div>
                    @endif
                </div>
            </x-ui.card>
        ...
        <div>
            <x-ui.card header="{{ __('Status & Class Assignment') }}">
                        ...
                        <select name="class_group_id" id="class_group_id" class="input mt-1">
                            ...
                            @foreach($classGroups as $group)
                                <option value="{{ $group->id }}" @selected(($group->german_level->value ?? $group->german_level) === ($application->german_level->value ?? $application->german_level))>
                                    ...
                                    @if($group->teacher) - {{ __('Teacher') }}: {{ $group->teacher->user->name ?? '' }} @endif
                                </option>
                            @endforeach
                        </select>
                        <p class="text-xs text-gray-500 mt-1">{{ __('Approving creates a student account and assigns them to the chosen class group.') }}</p>
                    </div>

                    <div>
                        <label for="notes" class="label font-medium text-gray-700">{{ __('Admin Notes') }}</label>
                        <textarea name="notes" id="notes" rows="4" class="input mt-1" placeholder="{{ __('Optional internal review notes...') }}">{{ old('notes', $application->admin_notes) }}</textarea>
                    </div>

                    <div class="pt-2">
                        <button type="submit" class="btn-primary w-full">{{ __('Save & Process Approval') }}</button>
                    </div>
                </form>
            </x-ui.card>
        </div>

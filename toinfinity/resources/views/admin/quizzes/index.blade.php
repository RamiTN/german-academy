<x-app-layout>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">{{ __('Manage quizzes and assessments.') }}</p>
        </div>
        <a href="{{ route('admin.quizzes.create') }}" class="btn-primary text-sm">
                    ...
                    <tbody>
                        @foreach($quizzes as $quiz)
                            <tr class="border-b border-gray-100 hover:bg-gray-50/50">
                                ...
                                <td class="px-4 py-3 text-gray-500">{{ $quiz->time_limit_minutes }} {{ __('mins') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        @else
            <div class="p-8 text-center">
                <div class="flex flex-col items-center">
                    ...
                    <p class="text-gray-500 text-sm max-w-sm mb-4">{{ __('Create quizzes to assess students\' understanding of the material.') }}</p>
                    <a href="{{ route('admin.quizzes.create') }}" class="btn-primary text-sm py-2 px-4">{{ __('Create Quiz') }}</a>
                </div>
            </div>
        @endif
    </x-ui.card>
